...
class MenuView(View):
    ...
    async def _send_dm_to_players(self, game):
        ...
        for pid in list(game.players):
            print(f"[DEBUG] Отправляю ЛС для {pid}")
            user = bot.get_user(pid) or await bot.fetch_user(pid)
            if not user:
                print(f"[ERROR] Не удалось найти пользователя {pid}")
                continue
            try:
                view = GameView(game, self.gm, pid)
                embed = self._make_embed(game, pid)
                print(f"[DEBUG] Пытаюсь отправить сообщение юзеру {user.name}...")
